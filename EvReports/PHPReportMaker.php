...
/* Function for Sql Query Path Content Loading 26-06-2018 */
		function getQueryComplete($sRst)
		{
			//return $sRst;
			//echo $sRst;
			if (strpos($sRst, '[PATH:') !== false) 
			{	
				$li_s = strpos($sRst, '[PATH:');
				$li_e = strpos($sRst, ']', $li_s);
				$ll_len = $li_e - $li_s - 6;
				$ls_path = substr($sRst, $li_s + 6, $ll_len);
				//$ls_path = $ls_path;
				//echo $ls_path;
				//echo getcwd();
				if (file_exists($ls_path)) 
				{
					$ls_data = file_get_contents($ls_path);
					//echo $ls_data; 
					$sRst = str_replace("[PATH:".$ls_path."]",$ls_data,$sRst);
					
					$sRst = $this->getQueryComplete($sRst);
				}
				else
				{
					echo "SQL Link File Not Find ".$ls_path;
				}
