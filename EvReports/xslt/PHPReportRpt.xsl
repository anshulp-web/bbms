...
<xsl:param name="sql"/>
<xsl:param name="mywhere"/>
<xsl:param name="mygrp"/>
<xsl:param name="myerror"/>
<xsl:param name="user"/>			
<xsl:param name="pass"/>
<xsl:param name="conn"/>
...
<xsl:template match="REPORT/MYWHERE">
</xsl:template>

<xsl:template match="REPORT/MYGRP">
</xsl:template>

<xsl:template match="REPORT/MYERROR">
</xsl:template>
...
<xsl:template name="DATACON">
	...
	$sWHR    = &quot;<xsl:value-of select="MYWHERE"/>&quot;;
	$sErr    = &quot;<xsl:value-of select="MYERROR"/>&quot;;
	...
	$oParameters    = Array();
	$oParameters = $oReport->getParameters();
	
	$sSQL = $sSQL . $sWHR . &quot;<xsl:value-of select="MYGRP"/>&quot;;
	
	foreach($oParameters as $x=>$x_value)
  	{
		//echo "val" . $x. "," . $x_value ;
		$sSQL = str_replace("[".$x."]","".$x_value."",$sSQL);
  	}
	...
	//$sSQL  = $sSQL. "and a.grp_cd = '" . $oReport->getParameter($sWHR) ."'";
	
	
	$err1 = substr($sErr, 0, 1);
	if ($err1 == 'Y')
	{
		echo $sSQL;
	}
	...
	if(is_null($this->_oCon))
		...
		$oCon = $this->_oCon;
    //Commented on 15-11-2020 Invalid Con Error
