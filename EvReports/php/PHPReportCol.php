	...
	*/
	class PHPReportCol extends PHPReportXMLElement {
		...
		*/		
		...
		function getShowHideRow($sr)
		{
			$ls_ret = "";
			$ls_val = $this->getSum("rsr") ;
		...
		function getSubReport3($RptNm, $ColList, $ColVal)
		{
			// $curl = curl_init();
			 //curl_setopt ($curl, CURLOPT_URL, "my.php");
			// curl_exec ($curl);
			 //curl_close ($curl);			
		...
		function getSubReport2($RptNm, $ColList, $ColVal)
		{

			$micro_date = microtime();
			$date = date('mdYhisa', time()).substr($micro_date, 3,5);
			$oRpt1 = new PHPReportMaker(); 
			$oRpt1->setXML('report/'.$RptNm);
			$oRpt1->_sClassName		= "PHPReport_".$date;
			//require_once("DBConnection.php");
			
			$db = new DBConnection();
			$oRpt1->setUser($db->user);
			$oRpt1->setPassword($db->pwd);
			$oRpt1->setDatabase($db->db);
			$oRpt1->setConnection($db->host);
			$oRpt1->setDatabaseInterface("mysql");
			//$oRpt->setParameters($param_obj);
			$oRpt1->_ResultHide = true;
			//$oRpt1->sCodeOut = "c:\abc.txt";
			//$oRpt1->run();
			//echo $oRpt1->getReportData();
			return $oRpt1->getReportData();
		}
		...
		function getSql($sql)
		{
			$ls_val = "";
			$Cn = new EvClass(); 
			$ls_val = $Cn->getExeSql($sql);
			return $ls_val;
		}

		function getSubReport($RptNm, $ColList, $ColVal)
		{
				$li_pos = -1;
				$li_s = -1;
				$ar1 = explode(",", $ColList);
				...
				$ary_obj = array();
				for ($i=0; $i < count($ls_col); $i++)
				{
					$ary_obj[$ls_col[$i]] = $ls_val[$i];
				} 
				$ls_retval = "?rpt_nm=" . $RptNm . "&parm_val=".json_encode($ary_obj);
				//echo $ls_retval;
				//$date = date('mdYhisa', time());
				$micro_date = microtime();
				$date = date('mdYhisa', time()).substr($micro_date, 3,5);
			
			echo "<script> function loadxml_".$date."(ls_file)
			{
				//alert(ls_file);
				var output = document.getElementById('DivTestRpt');
				var ele = document.createElement('div');
           		ele.setAttribute('id','tdiv_".$date."');
				output.appendChild(ele);
				
				//$('#DivTestRpt').load(\"my.php\" + ls_file);
				$('#tdiv_".$date."').load(\"my.php\" + ls_file);

				
				//var ls_h = $('#DivTestRpt').html();
				//ls_h = ls_h.substr(308);
				//alert(ls_h);
				//return ls_h;
			} 
			
			var ls_html = loadxml_".$date."('".$ls_retval."'); 
			
			</script>";
						
			
			//echo "<script> //var ls_html = loadxml_".$date."('".$ls_retval."'); 
			...
			type: 'post',
			url: 'aaa.php', 
			data: {ls_val: ls_html, ls_dt: '".$date."'},
			...
			//Session['myreport'] = ls_html; 
			//document.cookie=\"myreport=' + ls_html + \"'\";
			...
			*/
			...
			//echo "CCCCCCC:". $_COOKIE["myreport"];
			//return $_COOKIE["myreport"];
			
			//echo "fffffffffffffff";
			
			
			//$val = trim($_SESSION[$date]);
			
			//$val = substr($val , 0, strlen($val) );
			//echo $val;
